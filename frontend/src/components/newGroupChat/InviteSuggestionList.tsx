...
const InviteSuggestionList = ({ filteredFriends, onSelect }: InviteSuggestionListProps) => {
  ...
  return (
    <div className="border rounded-large mt-2 max-h-[180px] overflow-y-auto divide-y">
      {filteredFriends.map((friend) => (
        <div key={friend._id} className="flex items-center gap-3 p-2 cursor-pointer hover:bg-muted transition" onClick={() => onSelect(friend)}>
